...
def getUserPass(email,user_type):
    if user_type == 'civilian':
        ...
        try:
            query = "SELECT email,password,dep_id_app_user,name FROM dep_app_user WHERE email = '"+str(email)+"';"
            user_data = getServerData(query)
            return user_data['password'][0],user_data['dep_id_app_user'][0],user_data['name'][0],'department'
        except:
            return None,None,None,None
